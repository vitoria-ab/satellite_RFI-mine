...
def area(fname, file_path):
    ...
    if os.path.isfile(file_path + fname + "_Time_Pos.npy"):
        ...
        if fname in ["1551037708", "1551055211", "1553966342", "1554156377"]:
            data = katdal.open(
                "/idia/projects/hi_im/SCI-20180330-MS-01/"
                + fname
                + "/"
                + fname
                + "/"
        ...
        if fname in [
            ...
            data = katdal.open(
                "/idia/projects/hi_im/SCI-20190418-MS-01/"
                + fname
                + "/"
                + fname
                + "/"
        ...
        if fname in ["1558464584", "1558472940"]:
            data = katdal.open(
                "/idia/projects/hi_im/COM-20190418-MS-01/"
                + fname
                + "/"
                + fname
                + "/"
        ...
        if fname == "1562857793":
            ...
            data = katdal.open(
                "/idia/projects/hi_im/1562857793/1562857793/1562857793_sdp_l0.full.rdb"
            )
        ...
        #
        assert (
            data.corr_products[corr_id][0] == data.corr_products[corr_id][1]
        )  # check auto-corr
        ...
        if fname in ["1551037708", "1551055211", "1553966342", "1554156377"]:
            data1 = pickle.load(
                open(
                    "/idia/projects/hi_im/raw_vis/SCI-20180330-MS-01/"
                    + str(fname)
                    + "/"
                    + str(fname)
                    + "_"
        ...
        if fname in [
            ...
            data1 = pickle.load(
                open(
                    "/idia/projects/hi_im/raw_vis/SCI-20190418-MS-01/"
                    + str(fname)
                    + "/"
                    + str(fname)
                    + "_"
        ...
        if fname in ["1558464584", "1558472940"]:
            data1 = pickle.load(
                open(
                    "/idia/projects/hi_im/raw_vis/COM-20190418-MS-01/"
                    + str(fname)
                    + "/"
                    + str(fname)
                    + "_"
    ...
    if os.path.isfile(file_path + fname + "_nd_S0.npy"):
        ...
        ###
        f = 10.0  # this value can change the detecet jump
        ...
        for i in range(1, len(timestamps)):
            if (
                np.abs(vis[i, ch_plot0]) - np.abs(vis[i - 1, ch_plot0])
                > lim  # have a jump
                and vis[i, ch_plot0] > 0
                and vis[i - 1, ch_plot0] > 0
                and timestamps[i - 1] - timestamps[0] - data.dump_period / 2.0
                not in mark
            ):  # not jump the one before

                m = timestamps[i] - timestamps[0] - data.dump_period / 2.0
                # print i,m
