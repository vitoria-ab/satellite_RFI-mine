...
def unix_convert(tlist, sel):
    ...
    ot = tlist.utc[
        :, sel
    ]  # Selecting the YYYY-MM-DD and hh-mm-ss values from the list per point
    ot2 = ot.astype(int)
...
def remove_sats_below_horizen(sats, st_time, ed_time, location):
    ...
    for ii, key in enumerate(sats.keys()):
        ...
        if np.all(alt.radians < 0):
            ...
            alt_mask = ma.masked_array(alt.degrees, mask=alt.degrees < 0)
            tlist.append(
                unix_time(time_list=obs_time, choice=np.where(alt_mask.mask == True)[0])
            )
...
class Satellite_Catalogue(object):
    ...
    def get_sate_coords(self):
        ...
        for oo, obs_start_time in enumerate(self.obs_time):
            ...
            for ii, sat_type in enumerate(self.sats_type):
                ...
                sats, tlist_ = remove_sats_below_horizen(
                    sats, obs_time_list[0], obs_time_list[-1], self.obs_location
                )
                print(
                    "Satellite %12s has %4d satellites" % (sat_type, len(sats.keys()))
                ),
                if len(tlist_) == 0:
                    ...
                    (
                        ...
                        sats, obs_time_list, self.obs_location
                    )  # Edit
                    ...
                    for jj in range(len(tlist_)):
                        ...
                        try:
                            if int(tlist_[jj][0]) == 0:
                                mask_tlist = (
                                    self.obs_time_list[0].value <= tlist_[jj][-1]
                                )
                            else:
                                mask_tlist = (
                                    self.obs_time_list[0].value >= tlist_[jj][0]
                                )
                        except IndexError:
                            mask_tlist = np.zeros(
                                (len(self.obs_time_list[0].value)), dtype=bool
                            )

                        coord_list_sats_ma.append(
                            ma.masked_array(
                                coord_list_sats[:, jj, :],
        ...
        """
        self.sats_type_remain = [
            element
            for (i, element) in enumerate(self.sats_type)
        ...
        for rr in range(len(distance_list)):
            ...
            stand_in = np.zeros(
                (distance_list[rr].shape[0], distance_list[rr].shape[2])
            )
    ...
    def itersats_temperature(self, pointings, beam_func=None, close_angle=None):
        ...
        for oo, obs_start_time in enumerate(self.obs_time):
            ...
            obs_time_list = (
                obs_start_time.unix + self.obs_time_list[oo].to(u.second).value
            )
            obs_time_list = Time(obs_time_list, format="unix")
            obs_time_list.format = "fits"

            pointing_Az = pointings[oo][:, 0][:, None] * np.pi / 180.0
            pointing_Alt = pointings[oo][:, 1][:, None] * np.pi / 180.0
            ...
            for ii in tqdm(range(len(self.sats_type_remain))):
                ...
                _angle = np.sin(pointing_Alt) * np.sin(coords_Alt) + np.cos(
                    pointing_Alt
                ) * np.cos(coords_Alt) * np.cos(pointing_Az - coords_Az)
                _angle = np.arccos(_angle) * 180.0 / np.pi
                ...
                if beam_func is not None:
                    ...
                    _angle /= (
                        self.distance_list[ii].T ** 2
                    )  # Edit - divided the distance (r^2) from _angle
    ...
    def check_altaz(self):
        ...
        for oo, coord_list in enumerate(self.coord_list):
            ...
            for ii in range(len(self.sats_type_remain)):
                ...
                # names  = name_list[ii]
                ax.plot(
                    self.az_alt_pointings[:, 0] * np.pi / 180,
                    90 - self.az_alt_pointings[:, 1],
                    alpha=1,
                    color="#808080",
                    label="Coverage",
                )
                ...
                for jj in range(coords.shape[1]):
                    ...
                    ax.plot(
                        coords[good, jj, 0],
                        90.0 - coords[good, jj, 1] * 180.0 / np.pi,
                        ".-",
                        color=_c_list[ii],
                        mfc=_c_list[ii],
                        mec="none",
                ...
                legend_list.append(
                    mpatches.Patch(
                        color=_c_list[ii], label="%s" % self.sats_type_remain[ii]
                    )
                )

            legend_list.append(mpatches.Patch(color="#808080", label="Coverage"))
    ...
    def check_pointing(self, rising_altaz, setting_altaz, az_range=18.0, pad=2.0):
        """
        rising_altaz = [Az, Alt]
        ...
        for ii in range(len(self.sats_type_remain)):
            ...
            for jj in range(coords_1.shape[1]):
                ...
                az = coords_1[good, jj, 0] * 180.0 / np.pi
                alt = coords_1[good, jj, 1] * 180.0 / np.pi
                # az[az>180.] -= 360.
                ax_r.plot(
                    az,
                    alt,
                ...
                good = coords_1[:, jj, 0] * 180.0 / np.pi > _az0 - pad
                good *= coords_1[:, jj, 0] * 180.0 / np.pi < _az0 + az_range + pad
                good *= coords_1[:, jj, 1] * 180.0 / np.pi > _alt0 - pad - 0.5
                good *= coords_1[:, jj, 1] * 180.0 / np.pi < _alt0 + pad + 0.5
                if np.any(good):
                    az = np.mean(coords_1[good, jj, 0] * 180.0 / np.pi)
                    alt = np.mean(coords_1[good, jj, 1] * 180.0 / np.pi)  # + shift1
                    _sats_list1.append([az, alt, r"%s" % names_1[jj], _c_list[ii]])

            for jj in range(coords_2.shape[1]):
                good = coords_2[:, jj, 1] > 0
                az = coords_2[good, jj, 0] * 180.0 / np.pi
                alt = coords_2[good, jj, 1] * 180.0 / np.pi
                ax_s.plot(
                    az,
                    alt,
                    ".-",
                ...
                good = coords_2[:, jj, 0] * 180.0 / np.pi > _az1 - pad
                good *= coords_2[:, jj, 0] * 180.0 / np.pi < _az1 + az_range + pad
                good *= coords_2[:, jj, 1] * 180.0 / np.pi > _alt1 - pad - 0.5
                good *= coords_2[:, jj, 1] * 180.0 / np.pi < _alt1 + pad + 0.5
                if np.any(good):
                    az = np.mean(coords_2[good, jj, 0] * 180.0 / np.pi)
                    alt = np.mean(coords_2[good, jj, 1] * 180.0 / np.pi)
                    _sats_list2.append([az, alt, r"%s" % names_2[jj], _c_list[ii]])

            legend_list.append(
                mpatches.Patch(color=_c_list[ii], label="%s" % self.sats_type[ii])
            )
        shift = 0.015
        ...
        ax_s.legend(
            handles=legend_list, frameon=False, markerfirst=False, loc=1,
        )
        ...
        ax_r.set_xlim(_az0 - 10.0, _az0 + az_range + 10.0)
        ...
        ax_s.set_xlim(_az1 - 10.0, _az1 + az_range + 10.0)
        ...
        ax_r.set_ylim(_alt - 5.0, _alt + 5.0)
        ax_s.set_ylim(_alt - 5.0, _alt + 5.0)
        plt.show()
...
class Telescopesite_Satellite_Catalogue(Satellite_Catalogue):
    ...
    def check_pointing(self, altaz, figaxes=None, pad=2.0):
        """
        rising_altaz = [Az, Alt]
        setting_altaz = [Az, Alt]
        """
        ...
        for ii in range(len(self.sats_type)):
            ...
            for jj in range(coords_1.shape[1]):
                ...
                good = coords_1[:, jj, 0] * 180.0 / np.pi > _az_min - pad
                good *= coords_1[:, jj, 0] * 180.0 / np.pi < _az_max + pad
                good *= coords_1[:, jj, 1] * 180.0 / np.pi > _alt_min - pad - 0.5
                good *= coords_1[:, jj, 1] * 180.0 / np.pi < _alt_max + pad + 0.5
                if np.any(good):
                    az = np.mean(coords_1[good, jj, 0] * 180.0 / np.pi)
                    alt = np.mean(coords_1[good, jj, 1] * 180.0 / np.pi)
                    _sats_list1.append([az, alt, r"%s" % names_1[jj], _c_list[ii]])
        ...
        ax_r.set_xlim(_az_min - 10.0, _az_max + 10.0)
        ax_r.set_ylabel(r"${\rm Alt}\, [^{\circ}]$")
        ax_r.set_xlabel(r"${\rm AZ}\, [^{\circ}]$")
        ax_r.tick_params(which="both", direction="in")
