...
def gnss_satellites(name_sat, frequency_gnss, attenuation, sat_cat_data):
    ...
    for i in data_sub.index:
        ...
        if data_sub["P_t (dBW)"][i] == 0 or data_sub["G_t (dBi)"][i] == 0:
            ...
            power = (
                10 ** (data_sub["P_t (dBW)"][i] / 10)
                * 10 ** (data_sub["G_t (dBi)"][i] / 10)
                * data_sub["Alpha"][i]
                / (4 * np.pi)
            )  # Edit *r**2)
        ...
        if "BPSK(" in data_sub["Modulation"][i]:
            ...
            T_c = float(
                s[s.find("(") + 1 : s.find(")")]
            )  # Getting the chip interval value
            model = psd.BPSK(
                f=frequency_gnss - data_sub["Frequency[MHz]"][i],
                n_c=T_c,
        ...
        if "BOC(" in data_sub["Modulation"][i]:
            try:
                ...
                T_s, T_c = [
                    float(x) for x in s[s.find("(") + 1 : s.find(")")].split(",")
                ]
                model = psd.BOC(
                    f=frequency_gnss - data_sub["Frequency[MHz]"][i],
                    n_c=T_c,
        ...
        if "TMBOC" in data_sub["Modulation"][i]:
            ...
            # Getting the values between parenthesis and converting to float
            T_s, T_c, rt = [
                floaty(x) for x in s[s.find("(") + 1 : s.find(")")].split(",")
            ]
            model = psd.TMBOC(
                f=frequency_gnss - data_sub["Frequency[MHz]"][i],
                n_c=T_c,
        ...
        if attenuation[0] == None:
            ...
            # Adding Top-Hat
            model2 = model * af.tophat_rect(
                f=frequency_gnss,
                fi=data_sub["Frequency[MHz]"][i],
                band=attenuation[0],
                level=attenuation[1][i],
                values=np.ones(len(model)),
            )

        elif attenuation[0] == "goob":
            # Adding Gaussian Out-of-Band
            model2 = model * af.gaussian_oob(
                f=frequency_gnss,
                fi=data_sub["Frequency[MHz]"][i],
                band=data_sub["Bandwidth"][i],
                sigma=data_sub["Sigma"][i],
                values=np.ones(len(model)),
            )
        else:
            break
...
def TOD_sats(name_tod, fname, frequency_tod, beam_model, attenuation, sat_cat_data):
    ...
    sats_model, flux_density = gnss_satellites(
        name_sat=name_tod,
        frequency_gnss=frequency_tod,
        attenuation=attenuation,
        sat_cat_data=sat_cat_data,
    )
