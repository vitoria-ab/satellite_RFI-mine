...
class satellite_sim:
    ...
    def __init__(
        ...
        self.nd_s0, self.nd_s0_coords, self.frequency = (
            survey_info[0],
            survey_info[1],
            survey_info[2],
        )
        self.sat_data = pd.read_csv(
            sat_info, header=0, engine="python"
        )  # Get the chi2 here
        ...
        ) = self.get_satellite_angle_seperation()
        self.satellite_type, self.satellite_angle = specific_cons(
            constellation=self.cons,
            satellite_list=self.satellite_type,
            angle_list=self.satellite_angle,
        )
        ...
        self.sat_data = self.sat_data[
            self.sat_data["Sys"].str.contains("|".join(self.cons))
        ]
        ...
        if self.nearby_satellites_path != None:
            self.nearby_sats_time = pickle.load(
                open(self.nearby_satellites_path, "rb"), encoding="latin1"
            )
        else:
    ...
    def excecute(
        ...
        obs_time_start=None,
        obs_time_end=None,
        obs_frequency_start=None,
        obs_frequency_end=None,
        frequency_idx=None,
        file_bias_choice=None,
        add_sub=[True, False],
        attenuation_func=None,
        attenuation_sigma=None,
        bandsize=None,
        verbose=False,
    ):
        """
        ...
        self.sat_data.drop(
            (self.sat_data[self.sat_data["Frequency[MHz]"] > obs_frequency_end].index),
            inplace=True,
        )
        self.sat_data.drop(
            (
                self.sat_data[
                    self.sat_data["Frequency[MHz]"] < obs_frequency_start
                ].index
            ),
            inplace=True,
        )
        ...
        self.time_idx, self.frequency_idx = self.get_slice_idx(
        ...
        if self.nearby_sats_time != None:
            ...
            self.mask_nearby_satellites_slice = self.mask_nearby_satellites[
        ...
        if self.sats_only == False:
            ...
            (
                ...
                self.calibration_data_original,
                self.calibration_data_noise,
            ) = self.get_calibration_data()

            # Calibration data slice
            (
                self.calibration_data_slice,
                self.calibration_noise_slice,
            ) = self.get_data_slice()

        # Satellite simulation slice
        (
            self.simulation_slice,
            self.simulation_TOD_slice,
            self.bias_choice,
            self.satellite_TOD_slice,
        ) = self.get_simulation_slice(file_bias_choice=file_bias_choice)
    ...
    def get_frequency_information(self):
        ...
        if self.frequency_choice[0] == None and self.frequency_choice[1] == None:
            ...
            f_start_idx = (
                np.where(self.frequency > self.frequency_choice[0])[0][0]
            ) - 1
            f_end_idx = -1
    ...
    def get_calibration_data(self):
        ...
        try:
            ...
            data = pickle.load(
                open(self.data_loc + str(fname) + "_average_TOD_BG_model.p", "rb"),
                encoding="latin1",
            )
            ...
            if self.sub_BG == True:
                Temp_res = (
                    Temp_tod - Temp_noise
                )  # Getting the BG model to be added to the simualtions instead
    ...
    def get_satellite_angle_seperation(self):
        ...
        try:
            ...
            Satellite_type = data[
                "sat_name"
            ]  # Contains the names of the constellations
    ...
    def get_gnss_simulation(self):
        ...
        satellite_TOD = np.array(
            [
                gm.TOD_sats(
                    name_tod=satellite_name,
                    fname=self.file_name,
                    frequency_tod=self.frequency_band,
                    beam_model=self.satellite_angle[i],
                    attenuation=self.attenuation,
                    sat_cat_data=self.sat_data,
                )[0]
                for i, satellite_name in enumerate(self.satellite_type)
            ]
        )

        satellite_SED = np.array(
            [
                gm.TOD_sats(
                    name_tod=satellite_name,
                    fname=self.file_name,
    ...
    def get_mask_nearby_satellites(self, bandsize=None):
        ...
        for con_b in self.cons:
            ...
            if con_b.lower() in combined_con_list:
                frequency_vals = np.unique(
                    self.sat_data[self.sat_data["Sys"].str.contains(con_b)][
                        "Frequency[MHz]"
                    ].values
                )  # this line sorted
                idx = [
                    idx
                    for idx, con_i in enumerate(self.nearby_sats_time.keys())
                    if con_b.lower() in con_i
                ][0]
                ...
                masked_array = (
                    np.zeros(self.frequency_band.shape)
                    * np.zeros(self.nd_s0.shape)[:, np.newaxis]
                )
                ...
                # Makes a block for the mask data
                    ...
                    frequency_idx = [
                        np.where(fval > self.frequency_band)[0][-1] + 1
                        for fval in frequency_vals
                    ]  # getting the idx of the frequency values for 1 constellation
                    frequency_range_idx = np.unique(
                        np.concatenate(
                            [
                                np.arange(-idx_range + fval, idx_range + fval)
                                for fval in frequency_idx
                            ]
                        )
                    )
                    ...
                    for fi in frequency_range_idx:
                        masked_array[
                            self.nearby_sats_time[constellation_nearby], fi
                        ] = 1
    ...
    def get_slice_idx(
        self, start_time=None, end_time=None, start_frequency=None, end_frequency=None
    ):
    ...
    def get_data_slice(self):
        ...
        """
        calibration_data_slice = self.calibration_data[
            self.frequency_idx[0] : self.frequency_idx[1],
            self.time_idx[0] : self.time_idx[1],
        ]
        calibration_noise_slice = self.calibration_data_noise[
            self.frequency_idx[0] : self.frequency_idx[1],
            self.time_idx[0] : self.time_idx[1],
        ]
    ...
    def get_simulation_slice(self, file_bias_choice=None):
        ...
        satellite_TOD_slice = self.satellite_TOD[
            :,
            self.frequency_idx[0] : self.frequency_idx[1],
            self.time_idx[0] : self.time_idx[1],
        ]
        ...
        if type(file_bias_choice) == str:
            bias_choice = np.loadtxt(
                fname=self.bias_choice_loc + (file_bias_choice) + ".txt", delimiter=" "
            )
            ...
            print(
                "Enter the "
                + str(len(self.satellite_type))
                + " bias choices and 1 noise bias for the following: "
            )
        ...
        gnss_bias_model = np.nansum(
            [
                satellite_TOD_slice[i] * bias_choice[i]
                for i in range(len(satellite_TOD_slice))
            ],
            0,
        )  # + bias_choice[-1] Don't require this amplitude
        ...
        if self.add_BG == False:
            ...
            gnss_bias_model_m = (
                gnss_bias_model + self.calibration_noise_slice
            )  # Adding the calbiration noise levels

        gnss_bias_model_frequency = self._average_over_frequency_(gnss_bias_model_m)

        return (
            gnss_bias_model_frequency,
            gnss_bias_model_m,
            bias_choice,
            satellite_TOD_slice,
        )
