...
class data_reduction:
    ...
    def freqs_and_time(self):
        ...
        timestamps = (
            data_file.timestamps - data_file.timestamps[0]
        )  # Timestamps for full data [seconds]
    ...
    def get_gain_data(self, ant_no=0):
        ...
        gain_data_h = pickle.load(
            open(gain_data_loc + fname + "_" + recv_h + "_level3_data")
        )  ## hh-pol
        gain_data_v = pickle.load(
            open(gain_data_loc + fname + "_" + recv_v + "_level3_data")
        )  ## vv-pol
    ...
    def get_nd_times(self):
        ...
        area(
            fname=fname, file_path=self.folder_output
        )  # Calls a function to extract information from the data

        scan_time, scan_az, scan_el, scan_ra, scan_dec = np.load(
            self.folder_output + fname + "_Time_Pos.npy"
        )  # Full "scan" time, Azimuth and Elevation positions

        nd_off_scan_data = np.load(
            self.folder_output + fname + "_nd_S0.npy"
        )  # Extracting the Noise diode "off" time and position
        nd_off_scan_pos, nd_off_scan = (
            np.array(nd_off_scan_data[0], dtype="int64"),
            nd_off_scan_data[1],
        )

        nd_off_in_st_pos = np.array(
            [np.where(i == scan_time)[0][0] for i in nd_off_scan]
        )  # Getting the position of the nd_0 in the scan time period

        return (
            nd_off_scan - int(fname),
            nd_off_scan_pos,
            scan_time - int(fname),
            (scan_az[nd_off_in_st_pos], scan_el[nd_off_in_st_pos]),
            (scan_ra[nd_off_in_st_pos], scan_dec[nd_off_in_st_pos]),
        )
    ...
    def get_background_models(self, antenna, pol, mask_loc):
        ...
        T_rec = np.mean(
            pickle.load(
                open(mask_loc + self.file_name + "_" + antenna + "_t_rec.p", "rb")
            )[x],
            axis=0,
        )
        T_el = np.mean(
            pickle.load(
                open(mask_loc + self.file_name + "_" + antenna + "_t_el.p", "rb")
            )[x],
            axis=0,
        )
        T_gal = np.mean(
            pickle.load(
                open(mask_loc + self.file_name + "_" + antenna + "_t_gal.p", "rb")
            )[y],
            axis=0,
        )
        T_cmb = 2.73
    ...
    def conversion_rW_mK(
        self, antenna, pol, nd_off, frequency, c_start, c_end, mask_loc, fband=None
    ):
        ...
        # Noise models
        background_models = self.get_background_models(
            antenna=ant_name, pol=pol, mask_loc=mask_loc
        )
        ...
        # Mask
        path_2_masks = (
            "/idia/projects/hi_im/raw_vis/katcali_output/level1_output/mask/"
            + str(fname)
            + "_"
            + ant_name
            + pol
        ...
        if fband == None:
            ...
            # Formula
            a1 = (
                np.ma.array(visibility[nd_off, c_start:c_end], mask=mask)
                / g_t[:, None]
                / total_bg_models
            )  # New
            ...
            temperature_tod = (
                a1_true / freq_bandpass
            )  # * 1000 # Units mKelvin     [What did we do here????]

        else:
            f_band = pickle.load(
                open(mask_loc + fname + "_frequency_bandpass_" + ant_name + ".p", "rb")
            )  # Reading in the frequency bandpass
    ...
    def TOD(
        ...
        if fband == None:
            ...
            temperature_hh, frequency_h, gains_h, bg_models_h = self.conversion_rW_mK(
                antenna=ant_no,
                pol="h",
                nd_off=nd_off,
                frequency=frequency,
            ...
            temperature_vv, frequency_v, gains_v, bg_models_v = self.conversion_rW_mK(
                antenna=ant_no,
                pol="v",
                nd_off=nd_off,
                frequency=frequency,
