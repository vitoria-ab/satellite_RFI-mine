...
def radiometer_eq(data, n_dish):
    """
    Radiometer euquation for determining the error on the data
    data - the input data
    n_dish - the number of dishes available
    """
    d_nu = 0.2 * 10 ** 6  # Hz
    d_t = 2  # s
    n_pol = 2
        ...
        if waterfall.all() != None:
            if waterfall.ndim == 2:
                ...
                for i in range(intervals):
                    ...
                    new_waterfall = np.ma.array(
                        data=new_waterfall, mask=new_waterfall == 0
                    )
...
def time_line_masker(data_in, t_value):
    """
    Makes a mask the calibrated TOD across the timestamps for a 
    given pixel threshold.
    Parameters:
        data_in - Calibrated TOD data
        t_value - Threshold temperature value of choice
        
    Return: A mask for the TOD 
    """
    mask_idx = np.unique(np.where(data_in > t_value)[0])
    mask = np.zeros(data_in.shape)
    mask[mask_idx, :] = 1
    # data_mask = np.ma.array(data = data_in, mask = mask)
    return mask
