...
def gaussian_oob(f, fi, band, sigma, values):
    ...
    try:
        value_high = gaussian_taper(x=f[idx_high], a=values[idx_high[0]-1], b=f[idx_high][0], c=sigma)    # Looking at high side of gaussian tale
        value_low = gaussian_taper(x=f[idx_low], a=values[idx_low[-1]+1], b=f[idx_low][0], c=sigma)[::-1]

    except IndexError:
        try:
            value_high = gaussian_taper(x=f[idx_high], a=values[idx_high[0]], b=f[idx_high][0], c=sigma)       # Changed the positioning of the data
  
        except IndexError:
            value_low = gaussian_taper(x=f[idx_low], a=values[idx_low[-1]], b=f[idx_low][0], c=sigma)[::-1]
