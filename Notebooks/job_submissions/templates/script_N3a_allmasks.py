...
''' N3a_fitting: This script creates the simulation object, which gathers all the information, and fits the alphas to the observation given for all of the masking parameters pre-defined. '''
...
for typ in masks:
    for par in masks[typ]:
        ...
        for CF in CFs:
            ...
            if typ=="nomask":  
                fname = pm.my_name(folder=folder, CF=CF)
                data_info = {"CF":CF, "freq_slice":pm.freq_slice, "best-fit":alphas_BF}
            else:  
                fname = pm.my_name(**{"folder":folder, "CF":CF, typ:par})
                data_info = {"CF":CF, typ:par, "freq_slice":pm.freq_slice, 
                             "best-fit":alphas_BF}
...
for time_slice in [(775,1000), (2200,2400), (5500,6200)]:
    ...
    for CF in CFs:
        ...
        # saving information in the file
        fname = pm.my_name(folder=folder, CF=CF, time_slice=time_slice)
        data_info = {"CF":CF, "time_slice":time_slice, "freq_slice":pm.freq_slice, 
                     "best-fit":alphas_BF}
        with open(fname, "wb") as f:  pickle.dump(data_info,f)
